---
title: Coding Agent on Agent-Sandbox and LangGraph | Agent Sandbox
description: This guide documents the complete setup and deployment of a coding agent that runs locally on Kubernetes using Kind (Kubernetes in Docker) with the agent-sandbox controller. The agent uses locally-cached transformer models to generate and execute Python code with automatic error correction.
url: https://agent-sandbox.sigs.k8s.io/docs/use-cases/examples/langchain/
site: Agent Sandbox
...
You can find agent-sandbox setup instructions [here](/docs/getting_started/overview/#installation).
...
**Problem**: Takes 30-60 seconds to generate code. **Expected**: The 350M model runs on CPU without GPU acceleration. This is normal for local inference. **Options**:
