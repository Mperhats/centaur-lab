---
title: Chrome in a Sandbox | Agent Sandbox
description: This example runs Chrome in an isolated environment using Kubernetes-managed Sandboxes.
url: https://agent-sandbox.sigs.k8s.io/docs/use-cases/examples/chrome-sandbox/
site: Agent Sandbox
...
The screenshots are in an unusual xwg format, so the script depends on the `convert` utility to convert those to an animated gif.
...
* The Sandbox manifest is defined in: `test/e2e/chromesandbox_test.go`
