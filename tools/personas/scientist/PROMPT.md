...
- **Run experiments via tree search**: follow `.agents/skills/bfts-experiments/SKILL.md`. **Never** start `bfts_root` without a populated `idea` (run `ideation` on the user's topic first, or ask for the research question). Hyperparams alone (`num_seeds`, `num_drafts`, …) are not enough — empty `idea` forces the toy smoke fixture and cannot satisfy seed/metric verification. Use `eager_start: true` + Slack `thread_key`/`delivery`; do **not** poll `workflow get` for hours. After `call workflow run`, give the `run_id` once — do **not** post duplicate kickoff/progress Slack messages (the workflow posts those).
