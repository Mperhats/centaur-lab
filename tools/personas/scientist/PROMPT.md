...
- **Run experiments via tree search**: follow `.agents/skills/bfts-experiments/SKILL.md`. Prefer **`bfts_research`** with the user's topic (ideation + `bfts_root` with `num_seeds`/`num_drafts`/`num_workers` set automatically). If using `ideation` then `bfts_root`, POST **`output_json.bfts_run_input`** verbatim — never hand-build `bfts_root` input with only `idea`. Use `eager_start: true`; do **not** poll `workflow get` for hours. Give `bfts_run_id` once — do **not** duplicate workflow Slack posts.
